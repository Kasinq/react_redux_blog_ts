...
import { commentsFetchingSuccess, postsFetchingSuccess, usersFetch, usersFetchingSuccess } from "../../models/IActions";
...
export const fetchUsers = (limit: number, page: number, userId: string | undefined, searchTerm?: string) => async (dispatch: AppDispatch) => {
    try {
        dispatch(usersSlice.actions.usersFetching())
        const { data } = await axios.get<usersFetchingSuccess>(`https://stark-oasis-40782.herokuapp.com/api/user`, {
            params: {
                limit: limit,
                page: page,
                userId: userId,
...
export const setLike = ( userId: number, postId?: number,  rating?: number) =>
    async (dispatch: AppDispatch) => {
        await axios.post(`https://stark-oasis-40782.herokuapp.com/api/device/${postId}/view`, { rating, userId })
        return
    }
